...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow p-6">
        <form action="{{ route('admin.integrity.rules.update', $rule->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="target_role"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-300 focus:outline-none">
                    <option value="karyawan" {{ old('target_role', $rule->target_role) === 'karyawan' ? 'selected':'' }}>Karyawan</option>
                    <option value="admin"    {{ old('target_role', $rule->target_role) === 'admin' ? 'selected':'' }}>Admin</option>
                    <option value="all"      {{ old('target_role', $rule->target_role) === 'all' ? 'selected':'' }}>Semua</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="condition_type" id="condition_type"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-300 focus:outline-none">
                    <option value="menit_terlambat"  {{ old('condition_type', $rule->condition_type) === 'menit_terlambat' ? 'selected':'' }}>Menit Terlambat (dari jam shift)</option>
                    <option value="menit_lebih_awal" {{ old('condition_type', $rule->condition_type) === 'menit_lebih_awal' ? 'selected':'' }}>Menit Lebih Awal (sebelum jam shift)</option>
                    <option value="status_presensi"  {{ old('condition_type', $rule->condition_type) === 'status_presensi' ? 'selected':'' }}>Status Presensi (hadir/terlambat/alpa)</option>
                    <option value="jam_masuk"        {{ old('condition_type', $rule->condition_type) === 'jam_masuk' ? 'selected':'' }}>Jam Kedatangan Absolut</option>
                </select>
            ...
            <div class="flex items-center gap-2">
                ...
                <label class="text-sm font-semibold text-gray-700">Aktifkan Rule</label>
            </div>
